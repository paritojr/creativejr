...
class VideoLayer extends RenderedLayer {
  ...
  async render(ctx_out, ref_time) {
    ...
    const t = (ref_time - this.start_time) / 1000;
    if (t < 0 || t > this.video.duration) {
      return;
    }
    ...
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    if (Math.abs(this.video.currentTime - t) > 1 / 60) {
...
class Player {
  ...
  split() {
    ...
    if (this.time <= l.start_time) {
      return;
    }
    if (this.time >= l.start_time + l.total_time) {
      return;
    }
    const firstDuration = this.time - l.start_time;
    const secondDuration = l.total_time - firstDuration;
    const nl = new VideoLayer({
      name: l.name + "_split",
      _leave_empty: true
    });
    nl.video = document.createElement("video");
    nl.video.autoplay = false;
    nl.video.loop = false;
    nl.video.muted = true;
    nl.video.playsInline = true;
    nl.video.controls = false;
    nl.video.preload = "auto";
    nl.video.src = l.video.src;
    backgroundElem(nl.video);
    ...
    nl.total_time = firstDuration;

    l.start_time = this.time;
    l.total_time = secondDuration;

    nl.ready = false;
    nl.video.addEventListener('loadedmetadata', () => {
      nl.width = nl.video.videoWidth;
      nl.height = nl.video.videoHeight;
      nl.total_time = firstDuration;
      nl.ready = true;
    });
    
    if (nl.video.readyState >= 1) {
      nl.ready = true;
    }
    this.add(nl);
  }
...
function exportVideo(blob) {
  ...
  const text = document.createElement('div');
  text.innerHTML = `video exported!
  <br>
  <br>
  <video src="${vid.src}" width="466" height="300" controls></video>`;
  popup(text);
}
